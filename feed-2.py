...
Rules:
- Every item was published in the last 24 hours. Skip ones that only rehash older news.
- If several items cover the same story, post it once, from whichever source covers it best.
- Spread posts across sources: no more than 3 from any one source in a section.
...
def render(posts, alerts, notes, now_utc):
    ...
    for n, (label, chunk) in enumerate(content, 1):
        ...
        for p in chunk:
            ...
            summary = f'<p>{esc(p["summary"])}</p>' if p["summary"] else ""
            items.append(
                '<li><details>'
